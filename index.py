...
{{
  ...
  "all_experience": [
    {{
      "project_name": "Employer or client company name",
...
"""
    ...
    experience_summary[]:
      ...
      DETAILED SECTION KEYS (used by the Professional Experience section):
        project_name        → Employer or client company name ONLY (e.g., "GE Vernova"). Do NOT include the project name here.
        organisation        → Employer or client company name (should match project_name).
        ...
        key_activities      → Copy responsibility bullets from the resume AS-IS.
                              ...
                              Include EVERY bullet point listed under this role. Do NOT drop any key activities or responsibilities.
...
{{
  ...
  "experience_summary": [
    {{
      ...
      "project_name":         "Company name ONLY (e.g. GE Vernova)",
...
def sanitize_data(data):
    ...
    for entry in data["experience_summary"]:
        ...
        # Clean up hallucinations in skills string
        if isinstance(entry.get("skills"), str):
            # Model sometimes appends \nAchievements:, split by newline and take the first line
            skills_str = entry["skills"].split("\n")[0].strip()
            # Also remove trailing "Achievements:" if it was added without a newline
            if skills_str.lower().endswith("achievements:"):
                skills_str = skills_str[:-13].strip()
            entry["skills"] = skills_str
            
        # Ensure the header only displays the company name
        org = entry.get("organisation", "").strip()
        proj = entry.get("project_name", "").strip()
        if org:
...
def process_resume(pdf_path, template_path, output_path, status_callback=None):
    ...
    with open("debug_resume_data.json", "w", encoding="utf-8") as f:
